...
def removePerformanceLocation(titleString: str) -> str:
    stringsToRemove = ['PATIO: ', 'OUTPOST: ']
    for stringToRemove in stringsToRemove:
        titleString = titleString.replace(stringToRemove, '')
    return titleString
...
def getShowData() -> dict:
    dom = getWebpageAsDom('https://www.thegreyeagle.com/calendar')
    ...
    for i in range(showCount):
        ...
        shows.append({ 'artist' : removePerformanceLocation(getDomString(dom, titleXpath, i)), 
                       'date'   : date})
    return shows
